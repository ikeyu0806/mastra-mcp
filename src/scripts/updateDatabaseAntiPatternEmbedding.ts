...
概要:
複雑な問題を1つの長く複雑なSQLクエリで解決しようとするアンチパターン。
可読性・保守性が著しく低下し、バグの温床となる。
...
改善策:
- 問題を複数のステップに分割し、段階的に処理する
- 複数のシンプルなクエリを組み合わせることで処理の透明性を高める
- ビューや一時テーブル、ストアドプロシージャを活用する

これにより、保守性とパフォーマンスの両立が可能になる。

## インプリシットカラム（暗黙の列）

概要:
SELECT文でカラムをワイルドカード「*」で指定することは避けるべき。
暗黙的に全カラムを取得すると、予期せぬカラムが含まれたり、
不要なデータ取得によるパフォーマンス低下を招く。

問題点:
- 変更による影響範囲が分かりにくい
- ネットワーク負荷やメモリ消費が増加する
- セキュリティ上、不要な情報が漏れる可能性もある
...
概要:
パスワードを平文（プレーンテキスト）でデータベースに保存することは
重大なセキュリティリスクとなるアンチパターン。

問題点:
- 平文保存は漏洩時に即パスワードが流出してしまう
- ユーザーの他サービス利用も危険にさらす
- セキュリティコンプライアンス違反の可能性も高い

改善策:
- パスワードはソルト（salt）を付加して安全なハッシュ関数でハッシュ化し保存する
- ハッシュアルゴリズムはbcrypt、Argon2、PBKDF2など、計算コストの高いものを使う
- ソルトは各パスワードごとに異なる値を使うことで、レインボーテーブル攻撃を防ぐ

これにより、漏洩時のリスクを大幅に軽減できる。

## SQLインジェクション

概要:
動的に生成したSQL文に対し、ユーザー入力を適切に処理せずに直接組み込むことで、
悪意のあるSQLコードが実行されてしまうセキュリティ脆弱性。

問題点:
- 不正アクセスやデータの改ざん・漏洩の原因になる
- 管理者権限を奪取される可能性がある
- アプリケーション全体の信頼性を損なう

改善策:
- プリペアドステートメント（Prepared Statements）やパラメータ化クエリを使用する
- 入力値のエスケープやバリデーションを徹底する
- ORMやフレームワークの安全なクエリ機能を活用する
...
概要:
主キー（IDなど）をAUTOINCREMENTなどで連番に設定している場合に、
欠番（連番の飛び）を気にしてわざわざ埋めようとするアンチパターン。

問題点:
- 欠番を埋めるための処理は複雑化し、バグの温床になる
- 主キーの連続性は識別のために必須ではなく、無理に連続させる必要はない
- 欠番を埋める処理はパフォーマンスや保守性を損なう場合がある

改善策:
- 主キーは一意性と識別のためだけに使い、連続性は気にしない
- 欠番があっても問題がなければそのままにする
- 欠番埋めを目的にした複雑な処理は避ける

これによりシステムのシンプルさと安定性を保てる。

## シー・ノー・エビル（臭いものに蓋）

概要:
関数の戻り値やステータスコードを無視して処理を続けるアンチパターン。
問題が発生しているにも関わらずチェックを怠ることで、
バグや異常状態が見逃され、後続の処理に悪影響を及ぼす。

問題点:
- エラーや異常が検知できず、不具合の原因となる
- デバッグや障害対応が困難になる
- システム全体の信頼性が低下する

改善策:
- すべての戻り値や例外を必ずチェックする
- 異常があれば適切に例外処理やリカバリーを行う
- ステータスコードなどで状態を明示的に管理し、正常終了を保証する

これにより堅牢で信頼性の高いコードになる。

## ディプロマティック・イミュニティ（外交特権）

概要:
データベースはアプリケーションコードと同様に、品質保証(Quality Assurance: QA)に
真剣に取り組むべきであり、無敵の存在として扱ってはいけないというアンチパターン。
...
問題点:
- データベース設計や運用が雑になると、アプリケーション全体の品質低下を招く
- バグやパフォーマンス問題が見逃されやすい
- 保守性・拡張性に悪影響を及ぼす

改善策:
- データベース設計もQAプロセスの一環として扱う
- テストやレビューを体系的に実施する
- 要件に沿った設計・実装を心がける
...
概要:
性能や障害対応を十分に考慮せずに設計・実装されたシステムは、外から見れば立派でも、
一度トラブルが起きれば一瞬で崩壊する「砂の城」と化す。
安定運用のためには、現実的な負荷や障害を想定した設計と運用が不可欠。

対策ポイント:

1. ベンチマーク:
  - 処理性能や同時接続数などを事前に計測し、ボトルネックを特定しておく。

2. テスト環境の構築:
  - 本番環境と同等の構成を用意し、実運用に近い状態でテストを行う。

3. 例外処理の実装:
  - DBアクセスやAPI通信などで発生しうる例外に対して、適切なリカバリや通知を実装する。

4. バックアップ:
  - 定期的なバックアップの取得・検証を行い、万が一のデータ破損に備える。

5. 高可用性（HA: High Availability）:
  - サーバーやDBを冗長構成にし、単一障害点（SPOF）を排除する。

6. ディザスタリカバリ（DR: Disaster Recovery）:
  - 地震や火災などの災害時でもサービスを継続できるよう、別拠点の準備や自動フェイルオーバーを検討する。

7. 運用ポリシーの策定:
  - 障害対応フローや監視体制、復旧手順などを明文化し、チームで共有する。

これらの備えがなければ、立派に見えるサービスも実運用では脆く、すぐに崩れてしまう。
「壊れない設計」ではなく、「壊れても耐える設計」が重要。
